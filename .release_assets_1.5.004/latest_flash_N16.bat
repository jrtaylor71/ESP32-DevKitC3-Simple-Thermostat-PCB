...
esptool.py --chip esp32s3 --port %PORT% --baud 460800 --before default_reset --after hard_reset write_flash -z --flash_mode dio --flash_freq 80m --flash-size 16MB 0x0 "%LATEST_BUILD%\bootloader.bin" 0x8000 "%LATEST_BUILD%\partitions.bin" 0x10000 "%LATEST_BUILD%\firmware.bin"
