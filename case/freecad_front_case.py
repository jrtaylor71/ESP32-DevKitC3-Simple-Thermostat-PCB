...
display_width = 68.0
display_height = 50.0
display_x = pcb_clearance + display_center_x - display_width / 2.0
display_y = pcb_clearance + display_center_y - display_height / 2.0
...
sensor_box_wall = 1.5
sensor_x = pcb_clearance + 11.5
sensor_y = pcb_clearance + 68.42

corner_r = 4.0
face_edge_radius = 2.0  # Fillet radius at face/wall junction
...
# Top wall vents (before rotation: top = +Y direction)
for i in range(4):
    vent = Part.makeBox(sensor_box_width - 2, 4.0, 1.5)
    vent.translate(App.Vector(-(sensor_box_width - 2)/2, -(sensor_box_height/2), 2 + i * 2.5))
    try:
...
# Left wall vents (before rotation: left = -X direction)
for i in range(3):
    vent = Part.makeBox(4.0, sensor_box_height - 2, 1.5)
    vent.translate(App.Vector(-(sensor_box_width/2), -(sensor_box_height - 2)/2, 2 + i * 3))
    try:
        sensor_box = sensor_box.cut(vent)
    except:
        pass
...
# Fillet the display opening edges (outer edge at z=0)
try:
    display_fillet_radius = 1.0  # Small radius for display opening
    display_edges = []
    App.Console.PrintMessage("Searching for display opening edges to fillet...\n")
    ...
    App.Console.PrintMessage("Found %d display opening edges to fillet\n" % len(display_edges))
    if display_edges:
        shell = shell.makeFillet(display_fillet_radius, display_edges)
        App.Console.PrintMessage("Display opening fillet applied\n")
except Exception as ex:
    App.Console.PrintWarning("Display opening fillet failed: %s\n" % ex)
...
for hole in pcb_mount_holes:
    x = hole[0] + pcb_clearance
    y = hole[1] + pcb_clearance
...
# ---------- Cut latch windows in front walls ----------
...
for snap in snap_positions:
    ...
    pos = snap['pos']
    if side == 'top':
        window = Part.makeBox(latch_window_width, wall_thickness, latch_window_height)
        ...
        window = Part.makeBox(wall_thickness, latch_window_width, latch_window_height)
        window.translate(App.Vector(0,
    ...
    try:
        ...
        App.Console.PrintMessage("Cut latch window on %s wall\n" % side)
    except Exception as ex:
        App.Console.PrintWarning("Latch window cut failed (%s): %s\n" % (side, ex))
    ...
    for e in shell.Edges:
        if len(e.Vertexes) >= 2:
            ...
            if near(v1.z, face_thickness) and near(v2.z, face_thickness):
                ...
                v1_inner = (near(v1.x, -wall_thickness) or near(v1.x, -(case_length - wall_thickness)) or 
                           near(v1.y, wall_thickness) or near(v1.y, case_width - wall_thickness))
                v2_inner = (near(v2.x, -wall_thickness) or near(v2.x, -(case_length - wall_thickness)) or 
                           near(v2.y, wall_thickness) or near(v2.y, case_width - wall_thickness))
                
                if v1_inner and v2_inner:
                    inner_edges.append(e)
    
    App.Console.PrintMessage("Found %d inner edges to fillet with radius %.2f mm\n" % (len(inner_edges), face_edge_radius))
    if inner_edges:
        try:
            shell = shell.makeFillet(face_edge_radius, inner_edges)
            App.Console.PrintMessage("Inner fillet (reinforcement) applied successfully\n")
        except Exception as ex:
            App.Console.PrintWarning("Inner fillet failed: %s\n" % ex)
except Exception as ex:
